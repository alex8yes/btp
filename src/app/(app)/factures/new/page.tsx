...
import { Client, ClientType, CatalogueItem, LineItem, FactureStatus, TVARate } from '@/lib/types';
import { formatCurrency, generateLineId, calculateLineTotal } from '@/lib/utils';
import { Plus, Trash2, Save, Copy, ChevronRight, ChevronLeft, User, Calendar, ListChecks, FileText, X, CreditCard } from 'lucide-react';
...
export default function NewFacturePage() {
  ...
  const [settings, setSettings] = useState({ name: '', address: '', postalCode: '', city: '', email: '', phone: '', siret: '', defaultTVA: 20, legalMentions: '' } as any);
  ...
  const handleSave = () => {
    ...
    const saved = store.saveFacture(factureData);
    router.push(`/factures/${saved.id}`);
  };
  ...
  const handleSaveAndExportPDF = () => {
    ...
    const pdf = generateFacturePDF(saved, client, settings as any);
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 2:
        return (
          <Card>
            <h2 className="text-xl font-semibold font-[Outfit] mb-6">Dates de la facture</h2>
            ...
            <p className="mt-4 text-sm text-[var(--text-secondary)]">
              Par défaut, l'échéance est fixée à 30 jours.
            </p>
          </Card>
        );
      ...
      case 4:
        return (
          <Card>
            <h2 className="text-xl font-semibold font-[Outfit] mb-6">Paiement</h2>
            <div className="grid grid-cols-2 gap-6">
              <Select
                label="Statut de paiement"
                value={formData.status}
                onChange={(e) => setFormData({ ...formData, status: e.target.value as FactureStatus })}
                options={[
            ...
            <div className="mt-6 p-4 bg-[var(--bg-secondary)] rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-[var(--text-secondary)]">Nombre de lignes</span>
                  <span className="font-medium">{lines.length}</span>
                </div>
                <div className="flex justify-between border-t border-[var(--border)] pt-2 mt-2">
                  <span className="font-medium">Total TTC</span>
                  <span className="font-mono font-bold text-[var(--accent)]">{formatCurrency(totals.totalTTC)}</span>
                </div>
              </div>
            </div>
          </Card>
